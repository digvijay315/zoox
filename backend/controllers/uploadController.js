...
const uploadToCloudinary = async (req, res) => {
  try {
    ...
    const uploadPromises = fileArray.map((file) => {
      const originalName = path.parse(file.originalname).name.replace(/\s/g, "_") + "_" + Date.now();
    ...
    // Try to remove temp files even on error to prevent storage leaks
    try {
      if (files) {
        const fileArray = Array.isArray(files) ? files : [files];
        await Promise.all(fileArray.map((file) => fs.unlink(file.path).catch(() => {})));
      }
    } catch (err) {}
